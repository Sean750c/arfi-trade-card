...
interface FAQState {
  ...
  // Actions
  fetchFAQList: (token: string, refresh?: boolean) => Promise<void>;
  loadMoreFAQs: (token: string) => Promise<void>;
  clearFAQData: () => void;
}
...
export const useFAQStore = create<FAQState>((set, get) => ({
  ...
  fetchFAQList: async (token: string, refresh = false) => {
    const state = get();
  ...
  loadMoreFAQs: async (token: string) => {
    const state = get();
